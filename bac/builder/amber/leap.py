...
'''
Original tLEaP wrapper taken from YANK (Chodera Lab)
...
class TLeap:
    ...
    To avoid problems with special characters in file paths, the class runs the
    tleap script in a temporary folder with hardcoded names for files and then
    copies the output files to their respective folders.
    ...
    def __init__(self):
        ...
        # paths of input/output files to copy in/from temp dir
        self._file_paths = {}

        # parameter files already loaded
    ...
    def load_parameters(self, *args):
        ...
        Uses ``loadAmberParams`` for ``frcmod.*``/``*.frcmod`` files
        Uses ``loadOff`` for ``*.off`` and ``*.lib`` files
        Uses ``loadamberprep`` for ``*.prep``, ``*.prepi``, ``*.prepc`` files
        ...
        for par_file in args:
            ...
            elif extension in ['.prep', '.prepi', '.prepc']:
    ...
    def combine(self, unit_name, *args):
        """
        Add commands to combine units to the LEaP script
    ...
    def add_ions(self, unit_name, ion, num_ions=0, replace_solvent=False):
        """
        Add commands to script to add ions to a unit in LEaP
    ...
    def solvate(self, unit_name, solvent_model, clearance):
        """
        Add commands to solvate a unit isometrically to LEaP script
    ...
    def save_unit(self, unit_name, output_path):
        """
        Add commands to write a unit to file into isometrically script.
        Accepts the following formats: ``*.prmtop``, ``*.inpcrd``, or ``*.pdb`` files
